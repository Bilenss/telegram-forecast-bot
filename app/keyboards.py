...
from .keyboards import mode_keyboard, category_keyboard, pairs_keyboard, timeframe_keyboard, restart_keyboard
...
@dp.message_handler(lambda message: "Start over" in message.text or "Начать заново" in message.text, state="*")
async def restart_process(m: types.Message, state: FSMContext):
...
@dp.message_handler(lambda message: "Back" in message.text or "Назад" in message.text, state="*")
async def handle_back_button(m: types.Message, state: FSMContext):
    """Обработка кнопки 'Назад' для всех состояний"""
    ...
    if current_state == "ForecastStates:Category":
        ...
        await m.answer("Choose analysis mode:" if lang == "en" else "Выберите режим анализа:", 
                      reply_markup=mode_keyboard(lang))
        await ST.Mode.set()
        
    elif current_state == "ForecastStates:Pair":
        # Возврат к выбору категории
        await m.answer("Choose asset category:" if lang == "en" else "Выберите категорию актива:", 
        ...
        pairs = all_pairs(cat)
        await m.answer("Choose pair:" if lang == "en" else "Выберите валютную пару:", 
                      reply_markup=pairs_keyboard(pairs, lang))
        await ST.Pair.set()
        
    else:
        # Если неизвестное состояние, возврат к началу
...
async def set_timeframe(m: types.Message, state: FSMContext):
    ...
    try:
        ...
        # Показываем кнопку "Начать заново" при ошибке
        await m.answer("Try again:", reply_markup=restart_keyboard(lang))
        await state.finish()
        return
    ...
    try:
        ...
        # После успешного прогноза показываем кнопку "Начать заново"
        await m.answer("Analysis complete!", reply_markup=restart_keyboard(lang))
