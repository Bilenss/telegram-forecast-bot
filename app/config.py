...
# -----------------------
# HTTP-API for candles
# -----------------------
# Скопируйте точный XHR-URL из DevTools → Network → XHR → Request URL
...
def _mask_proxy(proxy: str) -> str:
    ...
    try:
        ...
        if u.username:
            auth = f"{u.username}:******"
            return f"{u.scheme}://{auth}@{u.hostname}:{u.port or ''}"
        return proxy
    except:
        return proxy

# -----------------------
# Print config summary at startup (without secrets)
# -----------------------
try:
